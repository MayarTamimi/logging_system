...
export async function getLogs(query : GetLogsQuery) {
    ...
    if(query.until) condition.push(gte(logs.timestamp ,  new Date(query.until)))
    if(query.q) condition.push(ilike(logs.message , `%${query.q}%`))

    for (const [key, value] of Object.entries(query)) {
        if (key.startsWith("attr.")) {
            const attributeKey = key.slice("attr.".length)
            condition.push(sql`${logs.attributes}->>${attributeKey} = ${String(value)}`)
        }
    }

    const res = await db.select().from(logs).where(
